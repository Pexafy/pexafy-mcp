...
class RootAliasedAuthProvider(RemoteAuthProvider):
    ...
    followed by the resource's own path — for this server,
    `/.well-known/oauth-protected-resource/mcp`. That is what FastMCP registers,
    and it is correct.

    Clients do not all agree. Several probe the bare
    `/.well-known/oauth-protected-resource` instead, and a 404 there is read as
    "this server has no authentication" — Glama classified this connector as
    "No Auth" on exactly that basis, then failed its connection test against a
    server that had answered 401 with a perfectly good `WWW-Authenticate` header
    pointing at the real document.

    So the same handler is mounted at the root path too. Not a second document:
    the route object generated by the parent is reused, so the two can never
    drift apart.
    """
